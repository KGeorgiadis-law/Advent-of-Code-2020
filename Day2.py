input_string = """1-7 q: qqqqxvqrkbqqztlqlzq
1-3 q: cqbm
...
pw_list = [s.split(":") for s in input_string.split("\n")]

def first_problem(pw_list):
  bad_passwords = []
  for rule, pw in pw_list:
    occ, letter = rule.split(" ")
    occ_min, occ_max = [int(o) for o in occ.split("-")]
    
    # to figure out how many times a substring appears in a string
    letter_occ = pw.count(letter)

    if letter_occ >= occ_min and letter_occ <= occ_max:
      bad_passwords.append([rule, pw])

  return(len(bad_passwords), bad_passwords[0], bad_passwords[-1])

def second_problem(pw_list):
  bad_passwords = []
  for rule, pw in pw_list:
    occ, letter = rule.split(" ")
    occ_1, occ_2 = [int(o) for o in occ.split("-")]

    try:
      occ_1_check = pw[occ_1] == letter
      occ_2_check = pw[occ_2] == letter
    except IndexError:
      print(occ_1, occ_2, len(pw), pw)

    # rule is: if occ1 or occ2 are true, then the rule is followed
    # however, if BOTH occ1 and occ2 are true, then the rule is breached

    if occ_1_check or occ_2_check:
      if occ_1_check and occ_2_check:
        bad_passwords.append([rule, pw])
    else:
      bad_passwords.append([rule,pw])

  return (len(pw_list) - len(bad_passwords), bad_passwords[0], bad_passwords[-1])
